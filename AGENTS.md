...
- **Persistence**: Log knowledge deltas using `ajourn log -m "..." -t "..."` after key decisions.
- **Vocabulary**: Structure entries with `DEC` (decisions), `RAT` (rationales), `GOT` (findings), `PRB` (problems), and `USR` (user requirements).

### Git Hygiene & Commit Standards
- Keep files like `.agents/` and `.jules/` git-ignored locally via [.git/info/exclude](file:///home/renich/src/crystal/.git/info/exclude).
- Never commit credentials or local build artifacts.
