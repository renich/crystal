...
class HTTP::StaticFileHandler
  ...
  private def check_request_path!(context : Server::Context, request_path : String) : Bool
    ...
    {% if flag?(:win32) %}
      # Prevent path traversal on Windows by rejecting backslashes in paths.
      # `\` is a valid character on Linux/macOS, but on Windows it acts as a directory separator.
