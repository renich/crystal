    ...
    def start
      ...
      agent_ws = PathWebSocketHandler.new "/agent" do |ws, context|
        if path = context.request.path
          if match_data = path.match(/\/(\d+)\/(\d+)$/)
            session_key = match_data[1].to_i
            tag = match_data[2].to_i
            Log.info { "#{path} WebSocket connected (session=#{session_key}, tag=#{tag})" }
            ...
            if session
              ...
              ws.close :normal, "Session not found"
            end
          else
            ws.close :normal, "Invalid agent path"
          end
        else
          ws.close :normal, "Missing request path"
        end
      end
        ...
        if !origin || !accept_request?(origin)
          ...
          ws.on_message do |message|
            begin
              ...
              when "run"
                if (source = json["source"]?.try(&.as_s?)) && (tag = json["tag"]?.try(&.as_i?))
