...
module Crystal
  class Program
    ...
    def compact_types(objects, &) : Array(Type)
      ...
      # ⚡ Bolt: Use a parallel Set to optimize O(N^2) `.includes?` lookups
      # down to O(1) during type union accumulation, avoiding significant
      # overhead for large unions while preserving insertion order.
      if objects.size > 15
