...
module Crystal
  class Program
    ...
    def compact_types(objects, &) : Array(Type)
      ...
      # Use a parallel Set to optimize O(N^2) include checks for large arrays of types.
      # We track `type.object_id` (UInt64) because `Type` uses reference equality.
      if objects.size > 15
        set = Set(UInt64).new(objects.size)
